...
module.exports = class PopupRequest {
    ...
    ApprovePopup(approveUrlPath) {
        ...
        const popupPath = '/externalproxy' + this.request.raw.originalUrl.split('?')[0];
        console.log(host, popupPath);
        const query = { ...this, request: undefined, reply: undefined, Result: undefined, popupURL: protocol + host + popupPath };
        this.Result["URL"] = protocol + host + '/externalproxy/PopupApprove/' + approveUrlPath + '?&data=' + jwt.sign(query, jwtKey);
        this.reply.send(this.Result);
    }
